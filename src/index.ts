...
export interface PersonalTokenConfig extends BaseConfig {
  token: string;
}
...
export class PipefyAPI {
  ...
  private get endpoint(): string {
    return this.config.endpoint || 'https://api.pipefy.com/graphql';
  }
  ...
  private get logTable(): string | null | undefined {
    return this.config.logTable;
  }
  ...
  private get intlCode(): string {
    return this.config.intlCode;
  }

  constructor(
    configOrApiKey: string | PipefyConfig,
    organizationId?: string,
    timeZone?: string,
    intlCode?: string,
    logTable: string | null = '',
  ...
  private async ensureValidToken(): Promise<string> {
    ...
    if (this.currentToken && this.tokenExpiresAt && Date.now() < this.tokenExpiresAt - 10000) {
      return this.currentToken;
    }
  ...
  async allCardsIds(
    ...
    const search: any = await (
      await this.pipefyFetch(
        `{ allCards(pipeId: ${pipeId}) { totalCount nodes { id${parentsQuery}${childenQuery} } pageInfo { hasPreviousPage hasNextPage } } } `,
      )
    ).json();
  ...
  updateFaseFields(cardId: string, fieldsToUpdate: any): Promise<Response> {
    ...
    return this.pipefyFetch(
      `mutation { updateFieldsValues(input: {nodeId: "${cardId}", values: [ ${fieldValues} ]}) { clientMutationId } }`,
    );
  }
  ...
  setLabels(cardId: string, labels: string[] | null): Promise<Response> {
    if (labels == null) {
      return this.pipefyFetch(
        `mutation { updateCard(input: {id: "${cardId}", label_ids: null }) { clientMutationId } }`,
      );
    } else {
      return this.pipefyFetch(
        `mutation { updateCard(input: {id: "${cardId}", label_ids: ["${labels.join('", "')}"]}) { clientMutationId } }`,
      );
    }
  }
  ...
  deleteTablerecord(recordId: string): Promise<Response> {
    return this.pipefyFetch(
      `mutation { deleteTableRecord(input: {id: "${recordId}"}) { clientMutationId success } }`,
    );
  }
  ...
  createCardRelation(
    ...
    return this.pipefyFetch(
      `mutation { createCardRelation(input:{ childId: ${childId}, parentId: ${parentId}, sourceId: ${sourceId}, sourceType: "${sourceType}" }) { cardRelation { id childId parentId } } }`,
    );
  }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (uploadResponse.ok) {
          ...
          console.error(
            `Error al cargar el archivo ${fileName}. Estado: ${uploadResponse.status} ${uploadResponse.statusText}`,
          );
        }
      }
      return null;
  ...
  async uploadFileFromBuffer(fileName: string, fileData: any): Promise<string | null> {
    try {
      ...
      console.log(
        `preSignedUrlResponse: ${preSignedUrlResponse.status}: ${preSignedUrlResponse.statusText}`,
      );
